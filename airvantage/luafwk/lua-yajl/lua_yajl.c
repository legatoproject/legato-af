...
static int js_generator_value(lua_State *L) {
    ...
    lua_getfield(L, LUA_REGISTRYINDEX, "yajl.null");
    if ( lua_equal(L, 2, -1) ) type = LUA_TNIL;
    lua_pop(L, 1);
...
static void js_create_null(lua_State *L) {
    int status;
    lua_getglobal(L, "require");                    // require
    lua_pushliteral(L, "niltoken");                 // require, "niltoken"
    status = lua_pcall(L, 1, 1, 0);                 // null or errmsg
    if( status) { /* niltoken not found */          // errmsg
        lua_pop(L, 1);                              // -
        js_create_null_mt(L);                       // -
        lua_newuserdata(L, 0);                      // null
        luaL_getmetatable(L, "yajl.null.meta");     // null, mt
        lua_setmetatable(L, -2);                    // null
    }
    lua_pushvalue(L, -1);                            // null, null
    lua_setfield(L, LUA_REGISTRYINDEX, "yajl.null"); // null
}
