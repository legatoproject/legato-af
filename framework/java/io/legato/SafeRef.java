...
// -------------------------------------------------------------------------------------------------
/**
 *  The SafeRef class is used to translate between Java object pointers and Legato style references
 *  that can safely be passed across process boundaries.
 *
 *  @code
 *  MyDataObject obj = new MyDataObject(...);
 *  long newRef = myRefMap.newRef(obj);
 *
 *  PassRefToClient(newRef);
 *  @endcode
 *
 *  Later when the client returns the ref for further processing, the ref is found using the get
 *  method.  If the ref is invalid then a null object is returned.
 */
// -------------------------------------------------------------------------------------------------
public class SafeRef<RefType>
{
     ...
     *
     *  @param newObject  The object to map.
     *
    ...
    {
        // Use the current ref ID and then advance by 2 in order to keep the refs on an odd number
     ...
     *
     *  @param ref  The safe reference to look up in the map.
     *
     ...
     *
     *  @param ref  The safe reference to remove from the map.
     */
    // ---------------------------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------------------------
    /**
     *  Clear out all of the objects protected by the ref map.  Once done all current refs are
     *  invalid.
     */
    // ---------------------------------------------------------------------------------------------
    public void clear()
