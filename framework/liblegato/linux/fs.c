 ...
 *  - LE_NOT_PERMITTED  A directory in the tree belongs to a Read-Only space or do not have access
 *                      right and cannot be created
...
)
{
    ...
    while ((slashPtr = strchr (slashPtr + 1, '/')))
    {
        ...
        {
            if ((EROFS == errno) || (EPERM == errno) || (EACCES == errno))
            {
