 ...
 */
//--------------------------------------------------------------------------------------------------
#define MCC_PROFILE_NAME_MAX_LEN    30
...
typedef struct le_mcc_Profile
{
    ...
    le_mcc_call_Ref_t              callRef;                          ///< Reference for current call, if connected
...
{
    char                            telNumber[LE_TEL_NMBR_MAX_LEN]; ///< Telephone number
...
{
    le_mcc_call_Ref_t    callRef;     ///< The call reference.
 ...
 */
//--------------------------------------------------------------------------------------------------
le_dls_List_t CallList = LE_DLS_LIST_INIT;
 ...
 */
//--------------------------------------------------------------------------------------------------
static le_mcc_call_Ref_t UpdateCallObject
(
...
)
{
    ...
    while (linkPtr != NULL)
    {
        ...
        if (callPtr->callId == (int16_t)eventPtr->callId)
        {
            ...
            if(eventPtr->event == LE_MCC_EVENT_TERMINATED)
            {
                callPtr->callId = -1;
            }
...
{
    le_mcc_call_Ref_t *callRef = reportPtr;
...
)
{
    ...
    if ((currProfilePtr->callRef = UpdateCallObject(dataPtr, currProfilePtr)) == NULL)
    {
        ...
        if ((dataPtr->event == LE_MCC_EVENT_INCOMING) && (CallHandlerCount))
        {
...
)
{
    ...
    MccProfilePool = le_mem_CreatePool("MccProfilePool", sizeof(struct le_mcc_Profile));
    ...
    ReferencePool = le_mem_CreatePool("MccReferencePool", sizeof(le_mcc_call_Ref_t));

    // BEGIN, TODO Profile creation should not belong to this module
    // Create a default Modem-Sim1 profile
    profilePtr = (struct le_mcc_Profile*)le_mem_ForceAlloc(MccProfilePool);
    ...
    profilePtr->callEventId = le_event_CreateId(eventName, sizeof(le_mcc_call_Ref_t));
...
)
{
    ...
    if (strlen((const char*)destinationPtr) > LE_TEL_NMBR_MAX_LEN)
    {
        LE_ERROR("Destination telephone number is too long: %zd",
                 strlen((const char*)destinationPtr));
        return NULL;
    }
 ...
 * Note tht 'len' sould be at least equal to LE_TEL_NMBR_MAX_LEN, otherwise LE_OVERFLOW error code
 * will be common.
 *
...
)
{
    ...
    {
        LE_KILL_CLIENT("Invalid reference (%p) provided!", callRef);
        return LE_MCC_TERM_NOT_DEFINED;
    }
...
)
{
    ...
    return (callPtr->txAudioStreamRef);
}
...
)
{
    ...
    return (callPtr->rxAudioStreamRef);
}
