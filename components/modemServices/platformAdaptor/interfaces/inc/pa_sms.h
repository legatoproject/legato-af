...
//--------------------------------------------------------------------------------------------------
typedef struct {
    le_sms_msg_Status_t status;                           ///< mandatory, status of msg in memory
    char                oa[LE_SMS_TEL_NMBR_MAX_LEN];      ///< mandatory, originator address
    char                scts[LE_SMS_TIMESTAMP_MAX_LEN];   ///< mandatory, service center timestamp
    le_sms_msg_Format_t format;                           ///< mandatory, SMS user data format
...
typedef struct {
    le_sms_msg_Status_t status;                           ///< mandatory, status of msg in memory
    char                da[LE_SMS_TEL_NMBR_MAX_LEN];      ///< mandatory, destination address
    uint8_t             data[LE_SMS_TEXT_MAX_LEN];        ///< mandatory, SMS user data
    uint32_t            dataLen;                          ///< mandatory, SMS user data length
}
pa_sms_SmsSubmit_t;
...
(
    le_sms_msg_Status_t    status, ///< [IN] The status of message in memory.
