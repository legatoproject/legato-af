...
)
{
    ...
    LE_DEBUG("%s: le_mdc_StartSession return code: %d", __func__, result);
    ...
    if (result == LE_OK)
    {
       // First wait a few seconds for the default DHCP client
        ...
        result = SetModemGateway();
        LE_DEBUG("%s: SetModemGateway return code: %d", __func__, result);
        if (LE_OK != result)
        {
    ...
    {
        LE_INFO("Data connection is already established.");
        return;
    }
    else
    {
...
{
    LE_DEBUG("%s: technology passed: %d", __func__, technology);
...
)
{
    ...
    result = le_mdc_GetSessionState(MobileProfileRef, &sessionState);
    LE_DEBUG("%s: result:  %d, sessionState: %d", __func__, result, sessionState);
    ...
    else
    {
        ...
        LE_INFO("%s: le_mdc_StopSession return code: %d", __func__, result);
        if (LE_OK != result)
        {
...
{
    LE_DEBUG("%s: technology passed :  %d", __func__, technology);
    LE_DEBUG("%s: connected passed  :  %d", __func__, connected);
