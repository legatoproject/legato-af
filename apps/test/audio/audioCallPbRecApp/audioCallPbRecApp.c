 ...
 * /tmp/legato/sandboxes/audioCallPbRecApp/record/tmp/record.wav
 *
 ...
 */
//--------------------------------------------------------------------------------------------------
static const char  AudioFileRecPath[] = "/record/tmp/record.wav";
...
)
{
    ...
    if (fchmod(AudioFileFd, 0444) == -1)
    {
        LE_WARN("Cannot set read rights to file %s: errno.%d (%s)",  AudioFileRecPath, errno, strerror(errno));
    }
