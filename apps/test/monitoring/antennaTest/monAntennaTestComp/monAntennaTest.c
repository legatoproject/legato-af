 ...
 * handler is unregistered, and the antenna is released.
 *
 *
...
)
{
    ...
    if (AntennaCtxtText[antennaType].count == NB_CHANGING_STATE_BEFORE_RELEASING)
    {
        ...
        result = le_antenna_Release(AntennaCtxtText[antennaType].antennaRef);
        LE_ASSERT(result == LE_OK);
    }
    else
    {
...
COMPONENT_INIT
{
    ...
    /*
     * Check to release: the release can't be proceeded as a handler has been subscribed
     */
    LE_INFO("Test release can't be proceeded: Error \"Handler is registered\" expected");
    result = le_antenna_Release (AntennaCtxtText[LE_ANTENNA_PRIMARY_CELLULAR].antennaRef);
