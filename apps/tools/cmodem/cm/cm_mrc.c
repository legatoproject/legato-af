...
)
{
    ...
    {
        case 0:
            cm_cmn_FormatPrint("Signal", "No signal strength");
            break;
        case 1:
            cm_cmn_FormatPrint("Signal", "Very weak signal strength");
            break;
        case 2:
            cm_cmn_FormatPrint("Signal", "Weak signal strength");
            break;
        case 3:
            cm_cmn_FormatPrint("Signal", "Good signal strength");
            break;
        case 4:
            cm_cmn_FormatPrint("Signal", "Strong signal strength");
            break;
        case 5:
            cm_cmn_FormatPrint("Signal", "Very strong signal strength");
            break;
        default:
...
)
{
    ...
    switch (rat)
    {
        ...
        case LE_MRC_RAT_CDMA:
            ...
            break;
        case LE_MRC_RAT_ALL:
            cm_cmn_FormatPrint("RAT", "All network (LE_MRC_RAT_ALL)");
            break;
        default:
...
*/
// -------------------------------------------------------------------------------------------------
static le_result_t GetHomeNetworkName
(
    void
)
{
    ...
    res = le_mrc_GetHomeNetworkName(homeNetwork, sizeof(homeNetwork));

    if (res != LE_OK)
    {
        cm_cmn_FormatPrint("Carrier", "");
        return res;
    }

    cm_cmn_FormatPrint("Carrier", homeNetwork);
...
{
    char configPath[512];
    snprintf(configPath, sizeof(configPath), "%s/%s", CFG_MODEMSERVICE_MRC_PATH, CFG_NODE_PREF_RAT);

    le_cfg_IteratorRef_t iteratorRef = le_cfg_CreateWriteTxn(configPath);
    le_cfg_SetString(iteratorRef, "0", rat);
